...
CREATE TABLE IF NOT EXISTS agent_insights (
  ...
  agent_type TEXT NOT NULL, -- 'sync_health', 'security_audit', 'bug_detection', etc.
  analysis TEXT NOT NULL, -- Full analysis from Claude
  severity TEXT NOT NULL CHECK (severity IN ('low', 'medium', 'high', 'critical')),
  metadata JSONB DEFAULT '{}', -- Additional context (store_id, entity_type, etc.)
...
CREATE INDEX idx_agent_alerts_is_read ON agent_alerts(is_read);
CREATE INDEX idx_agent_alerts_created_at ON agent_alerts(created_at DESC);

CREATE INDEX idx_agent_execution_log_agent_type ON agent_execution_log(agent_type);
CREATE INDEX idx_agent_execution_log_started_at ON agent_execution_log(started_at DESC);

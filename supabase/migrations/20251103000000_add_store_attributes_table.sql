...
CREATE INDEX idx_store_attributes_woo_id ON store_attributes(woo_id);
...
CREATE POLICY "store_attributes_manage"
ON store_attributes FOR ALL
TO authenticated
USING (
  has_role(auth.uid(), 'admin')
  OR user_has_store_access(auth.uid(), store_id)
...
-- Add global_attribute_id to product_attributes to link to global attributes

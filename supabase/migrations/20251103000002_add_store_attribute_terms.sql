-- Create store_attribute_terms table for global WooCommerce attribute terms
...
-- Create indexes
CREATE INDEX idx_store_attribute_terms_store_id ON store_attribute_terms(store_id);
CREATE INDEX idx_store_attribute_terms_attribute_id ON store_attribute_terms(attribute_id);
CREATE INDEX idx_store_attribute_terms_woo_id ON store_attribute_terms(woo_id);
...
-- Comments
COMMENT ON TABLE store_attribute_terms IS 'Available terms (values) for global WooCommerce attributes';
COMMENT ON COLUMN store_attribute_terms.attribute_id IS 'References the global attribute this term belongs to';
